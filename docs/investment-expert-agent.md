...
AI 投资专家是一个仅用于模拟盘的分钟级 Agent。它在每个交易日使用上一交易日及更早的数据选择候选，盘中读取 TickFlow 当日分钟 K，训练和回测读取独立的历史分钟 provider，并在收盘后基于可审计结果进行训练或策略进化。
...
3. 按日从设置中选择的“分钟数据源”下载历史未复权分钟线。日期 `T` 的分钟分区还包含 `T-1` 候选，以支持合法的跨日卖出与 T+1 标签。
...
- 训练与回测：使用「设置 -> 数据源」里选择的分钟 provider。历史源返回空日期、少于三根有效分钟线、缺少正常成交候选或混入其他日期时，任务立即失败，不写入伪完整分区。
- 当前 TickFlow Pro 官方覆盖为近一年分钟历史。选择 TickFlow 且请求三年时会在任务启动前直接阻止，不再运行一个必然失败的后台任务。
...
- 首次三年分钟数据构建会产生大量历史 provider 请求与磁盘占用，耗时取决于源端限流、候选数量和交易日数量。任务在单独后台工作线程串行运行，不阻塞 API。
