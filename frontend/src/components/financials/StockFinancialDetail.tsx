...
type FieldDef = { key: string; label: string; fmt: FmtType; group?: string }
...
const FIELD_DEFS: Record<TabKey, FieldDef[]> = {
  ...
  shares: [
    { label: '总股本', fmt: 'amount', key: 'total_shares' },
    { label: '流通股本', fmt: 'amount', key: 'float_shares' },
  ],
}
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-sm font-mono tabular-nums text-foreground text-right">
                          {formatValue(typeof val === 'number' ? val : null, def.fmt)}
                        </span>
                      </div>
                    )
                  })}
