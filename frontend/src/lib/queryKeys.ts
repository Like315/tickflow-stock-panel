...
export const QK = {
  ...
  usMarketGroups: ['us-market-groups'] as const,
  usMarketGroup: (kind: string, id: string) => ['us-market-group', kind, id] as const,
  usMarketInstruments: (query: string, sector: string, offset: number) =>
    ['us-market-instruments', query, sector, offset] as const,
  usMarketInstrument: (symbol: string) => ['us-market-instrument', symbol] as const,
