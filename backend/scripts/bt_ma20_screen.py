...
kline = (
    ...
    .collect().sort(["date", "symbol"])
)
kline = kline.filter(pl.col("date") <= END)
gn = pl.read_parquet(f"{DATA}/ext_data/ext_gn_ths/part.parquet")
tech_syms = set(r["symbol"] for r in gn.iter_rows(named=True) if any(k in str(r.get("所属概念") or "") for k in TECH_KEYWORDS))
...
idx = (
    ...
    .filter(pl.col("symbol") == "000688.SH").select(["date", "close"]).collect().sort("date")
)
...
liquidity = (
    k_warmup.group_by("symbol").agg(pl.col("amount").mean().alias("avg_amount"), pl.col("close").count().alias("n_days"))
    .filter(pl.col("n_days") >= 40).sort("avg_amount", descending=True)
)
...
prev_raw = np.zeros_like(raw_m); prev_raw[1:] = raw_m[:-1]
...
i0 = date2row[START]; i1 = date2row[END] + 1
r_cc = np.zeros_like(close_m); r_cc[1:] = close_m[1:] / close_m[:-1] - 1.0
...
class Sim:
    def __init__(self, cash=1_000_000.0):
        self.cash, self.mv, self.hold, self.equity, self.trades = cash, np.zeros(n_a), np.zeros(n_a, dtype=bool), [], 0
    def rebalance(self, t, target_idx):
        if self.hold.any():
            self.cash += self.mv[self.hold].sum() * (1 - COST_SELL)
            self.mv[self.hold] = 0.0; self.hold[:] = False; self.trades += 1
        ...
        if nb > 0 and self.cash > 1e4:
            ...
            self.cash -= budget * nb
            self.hold[can] = True; self.trades += nb
        dr = np.where(np.isfinite(np.where(self.hold, r_cc_f[t], 0.0)), np.where(self.hold, r_cc_f[t], 0.0), 0.0)
        self.mv *= (1 + dr)
    ...
    def stats(self):
        ...
        return {"total": total, "annual": ann, "max_dd": (eq / peak - 1).min(), "sharpe": sharpe, "trades": self.trades, "equity": eq.tolist()}
    ...
    if t == 0:
        ...
        if target == 1:
            ...
            if sim.hold.any():
                ...
                sim.mv[sim.hold] = 0.0; sim.hold[:] = False; sim.trades += 1
...
for t in range(len(dates_f)):
    if t == 0 or t in month_boundaries:
        sig = i0 + t - 1
        cond = np.isfinite(ma20[sig]) & np.isfinite(ma60[sig]) & (close_m[sig] > ma20[sig]) & (ma20[sig] > ma60[sig])
        sim.rebalance(t, cond)
    else:
        sim.hold_day(t)
...
for name, r in results.items():
    print(f"{name:34s} 总收益{r['total']*100:8.2f}% 年化{r['annual']*100:8.2f}% 回撤{r['max_dd']*100:8.2f}% 夏普{r['sharpe']:5.2f} 换手{r['trades']:5d}")

# ---- 当前时点候选名单（2026-08-13 收盘，演示该规则当下选出什么）----
t_last = len(dates_arr) - 1
sig = t_last
cond_now = np.isfinite(ma20[sig]) & np.isfinite(ma60[sig]) & (close_m[sig] > ma20[sig]) & (ma20[sig] > ma60[sig]) & (ma20[sig] > ma20[sig-5])
...
for i in cands[:25]:
    sym = pool[i]
    out_list.append({"symbol": sym, "name": str(name_map.get(sym, "")), "industry": hy_map.get(sym, "?"),
                     "close": round(float(close_m[sig, i]), 2),
                     "ma20": round(float(ma20[sig, i]), 2), "ma60": round(float(ma60[sig, i]), 2),
                     "pct_vs_ma20": round(float(close_m[sig, i] / ma20[sig, i] - 1) * 100, 2)})
    print(f"  {sym} {name_map.get(sym,''):8s} {hy_map.get(sym,'?'):10s} 收{close_m[sig,i]:.2f} MA20:{ma20[sig,i]:.2f} MA60:{ma60[sig,i]:.2f} 乖离{close_m[sig,i]/ma20[sig,i]-1:+.2%}")

json.dump({"results": {k: {kk: vv for kk, vv in v.items() if kk != 'equity'} for k, v in results.items()},
           "current_candidates": out_list},
          open(r"D:\MyTickFlowStockPanel\output\ma20_screen.json", "w", encoding="utf-8"), ensure_ascii=False, default=float)
